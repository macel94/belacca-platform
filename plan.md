...
- 2026-08-06: Implemented the separate `macel94/belacca-status` repository design: hourly GitHub-hosted external checks, sanitized v2 artifacts and history, Pong WebSocket journey reuse, freshness/unknown fallback, and explicit single-VM boundary. First child commit and parent submodule pin remain a manual reviewed publication step.
...
- Runtime gates remain open for external notification credentials, off-cluster backup storage/KMS, real isolated restore, authenticated dashboard synthetic coverage, GitHub registry attestation enforcement by digest, measured SLOs/burn-rate paging, and the first reviewed publication/pinning of the new status repository.
