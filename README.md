...
`belacca-production` tree. Native state now has a reliable AWS S3 backup destination with short-window
Object Lock (GOVERNANCE, 30-day default; relaxed on 2026-09-02), encryption,
scoped identities, synthetic acceptance evidence, and a USD 5 monthly budget
guard target. This is a reliable backup destination—not yet a production RPO
guarantee—because scheduled jobs
...
The site's [`status.html`](francesco-belacca-site/status.html) consumes a separate sanitized status artifact generated hourly by [`macel94/belacca-status`](https://github.com/macel94/belacca-status) from a GitHub-hosted runner outside the native cluster. Fresh observations are displayed; unavailable remote data is checked against the packaged bootstrap artifact and then falls back to unknown with an “awaiting fresh evidence” explanation. The status repository also publishes a reusable [`badge.json`](https://github.com/macel94/belacca-status/blob/main/badge.json) Shields endpoint artifact; its workflow badge is deliberately red when a target failure is successfully published. Reported uptime is calculated from good and bad critical observations and includes its observation count; no page response, build identifier, or empty incident list is treated as uptime evidence. The site's reliability page uses a sanitized, source-linked evidence ledger for backup, notification, and configuration claims rather than treating static prose as live runtime evidence.
